...
mycolors <- c("royalblue1","darkcyan","oldlace","palegreen","purple2","violetred1","wheat1","darkorange2")

for(i in 1:8)
{
  a <- cbind(as.data.frame(unlist(instances[[i]])),color=mycolors[i])
  
  print( scatter3D(x=a$V1,y=a$V2,z=a$V3,col =a$color))
...
summary(train_xyz_long)   #Scale needed?

pca_xyz <- prcomp(train_xyz_long%>%select(x,y,z), scale=TRUE)
tidy(pca_xyz,"pcs") #0.49 of the variance kept
pca_xyz$rotation

tidy(pca_xyz,matrix="rotation")%>% filter(PC == 1)
tidy(pca_xyz,matrix="eigenvalues") %>% filter(PC == 1)
